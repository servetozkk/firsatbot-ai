...
ok("VERSION 23.63.44", (root / "VERSION").read_text().strip() == "23.63.44")
ok("runtime endpoint", '/api/runtime-identity/v236344' in main)
ok("runtime constant", '_RUNTIME_VERSION_V236323 = "23.63.44"' in main)
ok("architecture", 'canonical-model-code-provenance-and-global-counter-integrity' in main)
...
# V23.63.44 general variant referential integrity guards.
...
ok("v236341 runtime endpoint", '/api/runtime-identity/v236344' in main_text)
for rel in [
    "main.py",
...
# V23.63.44 accessory/compatibility identity regression.
...
# V23.63.44 pseudo model-code provenance and counter integrity.
identity_text = (root / "app/services/product_identity_service.py").read_text(encoding="utf-8")
bulk_text = (root / "app/services/bulk_identity_service.py").read_text(encoding="utf-8")
integrity_text = (root / "app/services/model_code_counter_integrity_v236343_service.py").read_text(encoding="utf-8")
main_text = (root / "main.py").read_text(encoding="utf-8")
ok("pseudo model-code regex present", "PSEUDO_MODEL_CODE_PATTERN" in identity_text)
ok("pseudo extractor guard present", "_is_pseudo_model_code" in identity_text and "cls._is_pseudo_model_code(candidate)" in identity_text)
for dirty in ("uzunlugu1", "seviyesi5", "cozunurluk1024", "agirligi2", "2adet"):
    ok("pseudo code rejected " + dirty, ProductIdentityService._is_pseudo_model_code(dirty))
for legit in ("a2681", "a2941", "acs04236", "mnsk-8783", "b0b4k1dkgj", "pb200lzm", "82xb009gtx"):
    ok("real code preserved " + legit, not ProductIdentityService._is_pseudo_model_code(legit))
ok("bulk raw counter uses raw_products", "SELECT COUNT(*) FROM raw_products WHERE global_product_id=?" in bulk_text)
ok("bulk staging link counter removed", "SELECT COUNT(*) FROM bulk_identity_links WHERE global_product_id=?" not in bulk_text)
ok("bulk new global starts raw zero", "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,0,0,'ACTIVE',?,?)" in bulk_text)
ok("counter convergence service present", "run_model_code_counter_integrity_v236343" in integrity_text)
ok("authoritative raw counter", "RawProduct.global_product_id" in integrity_text and "func.count(RawProduct.id)" in integrity_text)
ok("authoritative active offer counter", "GlobalOffer.is_hidden.is_(False)" in integrity_text and "GlobalOffer.current_price > 0" in integrity_text)
ok("variant key rewrite deliberately zero", '"variant_key_rewrite_count": 0' in integrity_text)
ok("startup integrity hook", "run_model_code_counter_integrity_v236343()" in main_text)
ok("runtime model-code metadata", 'model_code_provenance_guard": "v23.63.43-specification-label-pseudo-codes-rejected"' in main_text)
ok("runtime counter metadata", 'global_counter_integrity": "v23.63.43-authoritative-raw-products-and-active-global-offers"' in main_text)
for rel in [
    "app/services/product_identity_service.py",
    "app/services/bulk_identity_service.py",
    "app/services/model_code_counter_integrity_v236343_service.py",
    "main.py",
]:
    ast.parse((root / rel).read_text(encoding="utf-8"))
    ok("AST v236344 " + rel, True)


# V23.63.44 source/canonical contradiction quarantine.
source_integrity = (root / "app/services/source_identity_integrity_v236344_service.py").read_text(encoding="utf-8")
source_policy = (root / "app/services/source_identity_policy_v236344.py").read_text(encoding="utf-8")
reconcile_text = (root / "app/services/catalog_reconciliation_service.py").read_text(encoding="utf-8")
ok("v236344 source integrity service", "run_source_identity_integrity_v236344" in source_integrity)
ok("v236344 semantic URL guard", "SEMANTIC_URL_PRODUCT_CLASS_CONFLICT" in source_policy)
ok("v236344 opaque Amazon guard", "_OPAQUE_URL_STORES" in source_policy and "amazon" in source_policy)
ok("v236344 canonical override GPU guard", "CANONICAL_OVERRIDE_GPU_CONFLICT" in source_policy)
ok("v236344 canonical override series guard", "CANONICAL_OVERRIDE_SERIES_CONFLICT" in source_policy)
ok("v236344 no new quarantine table", "CREATE TABLE" not in source_integrity and "CREATE TABLE" not in source_policy)
ok("v236344 production reconcile guard", "apply_source_identity_guard_v236344" in reconcile_text)
ok("v236344 raw quarantine not overwritten", "SOURCE_IDENTITY_QUARANTINED" in reconcile_text)
ok("v236344 guard before price history", reconcile_text.index("apply_source_identity_guard_v236344") < reconcile_text.index("record_global_offer_price("))
ok("v236344 startup hook", "run_source_identity_integrity_v236344()" in main_text)
ok("v236344 runtime endpoint", '/api/runtime-identity/v236344' in main_text)
ok("v236344 architecture", 'raw-source-canonical-contradiction-quarantine' in main_text)

# Synthetic policy checks do not require ORM/database dependencies.
from app.services.source_identity_policy_v236344 import evaluate_source_identity_values_v236344

def policy(store, title, url, canonical, payload="{}"): return evaluate_source_identity_values_v236344(
    store_code=store, source_url=url, raw_title=title, identity_payload=payload, canonical_name=canonical
)

v = policy("hepsiburada", "Tasarım Telefon Kılıfları", "https://www.hepsiburada.com/bingo-sivi-camasir-deterjani-p-X", "Tasarım Telefon Kılıfları")
ok("synthetic semantic URL corruption quarantined", v.quarantine and "SEMANTIC_URL_PRODUCT_CLASS_CONFLICT" in v.reasons)

v = policy("amazon", "Huawei FreeBuds SE 2 Beyaz", "https://www.amazon.com.tr/dp/B0CDMB5ZQW", "Huawei FreeBuds SE 2 Beyaz")
ok("synthetic Amazon opaque URL preserved", not v.quarantine)

v = policy("n11", "Monster Abra A5 RTX4050 32GB", "https://www.n11.com/urun/monster-abra-a5-rtx4050", "Monster Tulpar T6 RTX5060 16GB", '{"canonical_override": true}')
ok("synthetic canonical override GPU corruption quarantined", v.quarantine and "CANONICAL_OVERRIDE_GPU_CONFLICT" in v.reasons)

v = policy("amazon", "Xaser Sword X60 Ryzen RTX5060", "https://www.amazon.com.tr/dp/B0TEST", "Xaser Sword X55 Ryzen RTX5060", '{"canonical_override": true}')
ok("synthetic canonical override series corruption quarantined", v.quarantine and "CANONICAL_OVERRIDE_SERIES_CONFLICT" in v.reasons)

v = policy("trendyol", "Parfois Kadın Askılı Laptop Sırt Çantası", "https://www.trendyol.com/parfois/kadin-askili-laptop-sirt-cantasi-p-1", "Parfois Kadın Askılı Laptop Sırt Çantası")
ok("synthetic weak conflict fail-open preserved", not v.quarantine)

print(f"V23.63.44 MASTER smoke OK {len(checks)}/{len(checks)}")
