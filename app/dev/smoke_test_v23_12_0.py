...
def main() -> None:
    check((ROOT / "VERSION").read_text(encoding="utf-8-sig").strip() in {"23.12.0", "23.13.0"}, "v23.12 regression seti compatible VERSION")
    ...
    scraper_lost_mtm = p(
        name="Lenovo Ideapad Slim 3 Intel N100 4GB 128GB SSD W11 Notebook",
        brand="Lenovo",
        model="Ideapad Slim 3 Intel N100",
        category="Laptop",
        site="teknosa",
    )
    assert_red(lenovo, scraper_lost_mtm, "Scraper lost MTM without evidence")
    evidence_copy = p(
        name=scraper_lost_mtm.name + " Lenovo Ideapad Slim 3 82XB009GTX https://www.teknosa.com/...82xb009gtx...",
        brand=scraper_lost_mtm.brand,
        model=scraper_lost_mtm.model + " 82XB009GTX",
        category=scraper_lost_mtm.category,
        site="teknosa",
    )
    assert_match(lenovo, evidence_copy, "V23.12 preserved search-card URL/SKU evidence")
