    ...
    try:
        ...
        for offer, product_row, current_group in rows:
            ...
            if target_group.id == current_group.id:
                ...
                offer.match_reason = (
                    "V11.1 zorunlu varyant karantinası: "
                    + "; ".join(gate.reasons)
                )
                changes.append({
                    "offer_id": offer.id,
        ...
        db.commit()
        output = ROOT / "data" / "reports" / "v11_1_variant_repair.json"
        ...
        print(f"OK  Kontrol edilen teklif: {checked}")
        print(f"OK  Taşınan/karantinaya alınan teklif: {len(changes)}")
