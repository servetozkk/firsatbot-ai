...
ok("VERSION 23.63.43", (root / "VERSION").read_text().strip() == "23.63.43")
ok("runtime endpoint", '/api/runtime-identity/v236343' in main)
ok("runtime constant", '_RUNTIME_VERSION_V236323 = "23.63.43"' in main)
...
# V23.63.43 general variant referential integrity guards.
...
ok("v236341 runtime endpoint", '/api/runtime-identity/v236343' in main_text)
for rel in [
    "main.py",
...
# V23.63.43 accessory/compatibility identity regression.
...
# V23.63.43 pseudo model-code provenance and counter integrity.
...
for rel in [
    ...
    ast.parse((root / rel).read_text(encoding="utf-8"))
    ok("AST v236343 " + rel, True)

print(f"V23.63.43 ACCESSORY smoke extension OK {len(checks)}/{len(checks)}")
