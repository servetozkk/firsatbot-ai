...
ok("VERSION 23.63.41", (root / "VERSION").read_text().strip() == "23.63.41")
ok("runtime endpoint", '/api/runtime-identity/v236341' in main)
ok("runtime constant", '_RUNTIME_VERSION_V236323 = "23.63.41"' in main)
ok("architecture", 'global-variant-referential-convergence-and-raw-scoped-binding' in main)
...
ok("variant convergence active", 'variant_referential_convergence": "v23.63.41-active-offer-raw-variant-safe-convergence"' in main)
...
# V23.63.41 general variant referential integrity guards.
...
ok("v236341 runtime endpoint", '/api/runtime-identity/v236341' in main_text)
for rel in [
    "main.py",
...
print(f"V23.63.41 MASTER smoke OK {len(checks)}/{len(checks)}")
