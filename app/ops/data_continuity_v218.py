...
MARKER = ROOT / ".runtime" / "data_continuity_v218.json"
...
def _sqlite_backup(source: Path, destination: Path) -> None:
    ...
    tmp = destination.with_suffix(destination.suffix + ".v218tmp")
    if tmp.exists():
        tmp.unlink()
    src = sqlite3.connect(f"file:{source.as_posix()}?mode=ro", uri=True, timeout=15)
    dst = sqlite3.connect(tmp.as_posix(), timeout=15)
    try:
        src.backup(dst)
        dst.commit()
    finally:
...
def run() -> dict[str, Any]:
    ...
    if richest is not None and _score(richest) > _score(current):
        ...
        if CURRENT_DB.exists():
            backup = ROOT / "data" / "backups" / f"products-pre-v218-{timestamp}.db"
...
if __name__ == "__main__":
    ...
    if outcome.get("imported"):
        ...
        print("V21.8 veri devamlılığı: daha zengin önceki veritabanı devralındı.")
        print("Kaynak:", src.get("path"))
        print("Aktif GlobalOffer:", src.get("active_global_offers"))
    else:
        print("V21.8 veri devamlılığı: mevcut veritabanı korunuyor; daha zengin önceki DB bulunmadı.")
